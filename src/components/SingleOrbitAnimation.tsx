...
const SingleOrbitAnimation: React.FC<SingleOrbitAnimationProps> = ({ id, desc, isSelectedOrbit }) => {
    ...
    return (
        <Player key={id + "_lottie"} id={id + "_lottie"} src={animation} className={id + desc} autoplay={isSelectedOrbit ? isSelectedOrbit : false} loop style={{ opacity: isSelectedOrbit ? 1 : 0.25 }} />
    )
}
